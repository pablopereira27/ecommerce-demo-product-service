...
describe('Product Controller - Unit Tests', () => {
    describe('POST /products', () => {
        it('deve criar um produto válido', async () => {
            const res = await request(testApp()).post('/products').send({
                name: 'Notebook',
    ...
    describe('GET /products/:id', () => {
        it('deve retornar 404 ao buscar produto inexistente', async () => {
            const res = await request(testApp()).get('/products/9999');
    ...
    describe('DELETE /products/:id', () => {
        it('deve retornar 404 ao excluir inexistente', async () => {
            const res = await request(testApp()).delete('/products/9999');
            expect(res).toHaveStatus(404);
        });
    ...
    describe('GET /products', () => {
        it('deve listar com filtros aplicados', async () => {
            await request(testApp()).post('/products').send({
                name: 'Cadeira Gamer',
                description: 'Confortável',
                price: 1200,
            });
            const res = await request(testApp()).get('/products?name=Cadeira');
